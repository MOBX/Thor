...
/**
 * @author zxc Dec 31, 2015 6:23:06 PM
 */
public abstract class Mixin {
        ...
        try {
            ...
            for (int i = 0; i < ics.length; i++) {
                ...
                for (Method method : ics[i].getMethods()) {
                    ...
                    if (ix < 0) throw new RuntimeException("Missing method [" + desc + "] implement.");

                    Class<?> rt = method.getReturnType();
                    String mn = method.getName();
                    if (Void.TYPE.equals(rt)) ccp.addMethod(mn, method.getModifiers(), rt, method.getParameterTypes(),
                                                            method.getExceptionTypes(), "d" + ix + "." + mn + "($$);");
                    else ccp.addMethod(mn, method.getModifiers(), rt, method.getParameterTypes(),
    ...
    private static int findMethod(Class<?>[] dcs, String desc) {
        ...
        for (int i = 0; i < dcs.length; i++) {
            ...
            for (Method method : methods) {
                if (desc.equals(ReflectUtils.getDesc(method))) return i;
            }
        }
        return -1;
    }
    ...
    private static void assertInterfaceArray(Class<?>[] ics) {
        for (int i = 0; i < ics.length; i++)
            if (!ics[i].isInterface()) throw new RuntimeException("Class " + ics[i].getName() + " is not a interface.");
    }
}
