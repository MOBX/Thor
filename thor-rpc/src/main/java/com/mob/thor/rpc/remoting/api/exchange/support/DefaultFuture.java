...
public class DefaultFuture implements ResponseFuture {
    ...
    private static final Map<Long, ThorChannel>   CHANNELS = new ConcurrentHashMap<Long, ThorChannel>();
    ...
    public DefaultFuture(ThorChannel channel, Request request, int timeout) {
    ...
    private ThorChannel getChannel() {
        return channel;
    }
    ...
    public static boolean hasFuture(ThorChannel channel) {
        return CHANNELS.containsValue(channel);
    }

    public static void sent(ThorChannel channel, Request request) {
    ...
    public static void received(ThorChannel channel, Response response) {
        try {
    ...
    private String getTimeoutMessage(boolean scan) {
        ...
        return (sent > 0 ? "Waiting server-side response timeout" : "Sending request timeout in client-side")
               ...
               + (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(new Date()))
               + ","
               + (sent > 0 ? " client elapsed: " + (sent - start) + " ms, server elapsed: " + (nowTimestamp - sent) : " elapsed: "
                                                                                                                      + (nowTimestamp - start))
    ...
    static {
        Thread th = new Thread(new RemotingInvocationTimeoutScan(), "ThorResponseTimeoutScanTimer");
        th.setDaemon(true);
        th.start();
    }
}
