...
class CallbackServiceCodec {
    ...
    private static final ThorProtocol protocol             = ThorProtocol.getDubboProtocol();
    ...
    private static String exportOrunexportCallbackService(ThorChannel channel, URL url, Class clazz, Object inst,
        ...
        URL exporturl = new URL(ThorProtocol.NAME, channel.getLocalAddress().getAddress().getHostAddress(),
    ...
    private static Object referOrdestroyCallbackService(ThorChannel channel, URL url, Class<?> clazz, Invocation inv,
    ...
    private static String getServerSideCallbackServiceCacheKey(ThorChannel channel, String interfaceClass, int instid) {
        return Constants.CALLBACK_SERVICE_PROXY_KEY + "." + System.identityHashCode(channel) + "." + interfaceClass
               + "." + instid;
    }

    private static String getServerSideCallbackInvokerCacheKey(ThorChannel channel, String interfaceClass, int instid) {
    ...
    private static String getServerSideCountKey(ThorChannel channel, String interfaceClass) {
        return Constants.CALLBACK_SERVICE_PROXY_KEY + "." + System.identityHashCode(channel) + "." + interfaceClass
               + ".COUNT";
    }

    private static boolean isInstancesOverLimit(ThorChannel channel, URL url, String interfaceClass, int instid,
                                                boolean isServer) {
    ...
    private static void increaseInstanceCount(ThorChannel channel, String countkey) {
        try {
    ...
    private static void decreaseInstanceCount(ThorChannel channel, String countkey) {
        try {
            Integer count = (Integer) channel.getAttribute(countkey);
    ...
    public static Object encodeInvocationArgument(ThorChannel channel, RpcInvocation inv, int paraIndex)
                                                                                                        throws IOException {
        // encode时可直接获取url
    ...
    public static Object decodeInvocationArgument(ThorChannel channel, RpcInvocation inv, Class<?>[] pts,
        ...
        URL url = null;
        try {
            url = ThorProtocol.getDubboProtocol().getInvoker(channel, inv).getUrl();
        ...
        switch (callbackstatus) {
            ...
            case CallbackServiceCodec.CALLBACK_CREATE:
                try {
                    return referOrdestroyCallbackService(channel,
                                                         url,
                                                         pts[paraIndex],
                                                         inv,
                                                         Integer.parseInt(inv.getAttachment(INV_ATT_CALLBACK_KEY
                                                                                            + paraIndex)), true);
                } catch (Exception e) {
                    logger.error(e.getMessage(), e);
