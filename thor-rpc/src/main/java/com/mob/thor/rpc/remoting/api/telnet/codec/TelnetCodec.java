package com.mob.thor.rpc.remoting.api.telnet.codec;
...
import com.mob.thor.rpc.remoting.api.transport.codec.TransportCodec;
...
public class TelnetCodec extends TransportCodec {
    ...
    private static final List<?> EXIT              = Arrays.asList(new Object[] {
            new byte[] { 3 } /* Windows Ctrl+C */, new byte[] { -1, -12, -1, -3, 6 } /* Linux Ctrl+C */,
    ...
    protected Object decode(ThorChannel channel, ChannelBuffer buffer, int readable, byte[] message) throws IOException {
        if (isClientSide(channel)) {
        ...
        if (message[message.length - 1] == '\b') { // Windows backspace echo
            try {
                ...
                channel.send(new String(doublechar ? new byte[] { 32, 32, 8, 8 } : new byte[] { 32, 8 },
    ...
    private static Charset getCharset(ThorChannel channel) {
        if (channel != null) {
