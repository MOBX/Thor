...
public abstract class AbstractClient extends AbstractEndpoint implements Client {
    ...
    protected static final String                    CLIENT_THREAD_POOL_NAME  = "ThorClientHandler";
    ...
    public AbstractClient(URL url, ThorChannelHandler handler) throws RemotingException {
        super(url, handler);
        ...
        executor = (ExecutorService) ExtensionLoader.getExtensionLoader(DataStore.class).getDefaultExtension().get(Constants.CONSUMER_SIDE,
                                                                                                                   Integer.toString(url.getPort()));
        ExtensionLoader.getExtensionLoader(DataStore.class).getDefaultExtension().remove(Constants.CONSUMER_SIDE,
                                                                                         Integer.toString(url.getPort()));
    }

    protected static ThorChannelHandler wrapChannelHandler(URL url, ThorChannelHandler handler) {
    ...
    protected ExecutorService createExecutor() {
        return Executors.newCachedThreadPool(new NamedThreadFactory(CLIENT_THREAD_POOL_NAME
                                                                    + CLIENT_THREAD_POOL_ID.incrementAndGet() + "-"
                                                                    + getUrl().getAddress(), true));
    }
    ...
    public InetSocketAddress getRemoteAddress() {
        ThorChannel channel = getChannel();
     ...
     * @return channel
     */
    protected abstract ThorChannel getChannel();
