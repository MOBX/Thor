 ...
 * for one or more primitive byte arrays ({@code byte[]}) and {@linkplain ByteBuffer NIO buffers}.
 * <p/>
 * <h3>Creation of a buffer</h3>
 * <p/>
 * It is recommended to create a new buffer using the helper methods in {@link ChannelBuffers} rather than calling an
 * individual implementation's constructor.
 * <p/>
 ...
 * <p/>
 * {@link ChannelBuffer} provides two pointer variables to support sequential read and write operations -
 * {@link #readerIndex() readerIndex} for a read operation and {@link #writerIndex() writerIndex} for a write operation
 * respectively. The following diagram shows how a buffer is segmented into three areas by the two pointers:
 * <p/>
 * 
 * <pre>
 *      +-------------------+------------------+------------------+
 ...
 * <p/>
 * This segment is where the actual data is stored. Any operation whose name starts with {@code read} or {@code skip}
 * will get or skip the data at the current {@link #readerIndex() readerIndex} and increase it by the number of read
 * bytes. If the argument of the read operation is also a {@link ChannelBuffer} and no destination index is specified,
 * the specified buffer's {@link #readerIndex() readerIndex} is increased together.
 * <p/>
 * If there's not enough content left, {@link IndexOutOfBoundsException} is raised. The default value of newly
 * allocated, wrapped or copied buffer's {@link #readerIndex() readerIndex} is {@code 0}.
 * <p/>
 * 
 * <pre>
 ...
 * <p/>
 * This segment is a undefined space which needs to be filled. Any operation whose name ends with {@code write} will
 * write the data at the current {@link #writerIndex() writerIndex} and increase it by the number of written bytes. If
 * the argument of the write operation is also a {@link ChannelBuffer}, and no source index is specified, the specified
 * buffer's {@link #readerIndex() readerIndex} is increased together.
 * <p/>
 * If there's not enough writable bytes left, {@link IndexOutOfBoundsException} is raised. The default value of newly
 * allocated buffer's {@link #writerIndex() writerIndex} is {@code 0}. The default value of wrapped or copied buffer's
 * {@link #writerIndex() writerIndex} is the {@link #capacity() capacity} of the buffer.
 * <p/>
 * 
 * <pre>
 ...
 * {@code 0}, but its size increases up to the {@link #writerIndex() writerIndex} as read operations are executed. The
 * read bytes can be discarded by calling {@link #discardReadBytes()} to reclaim unused area as depicted by the
 * following diagram:
 * <p/>
 * 
 * <pre>
 ...
 * </pre>
 * <p/>
 * Please note that there is no guarantee about the content of writable bytes after calling {@link #discardReadBytes()}.
 * The writable bytes will not be moved in most cases and could even be filled with completely different data depending
 * on the underlying buffer implementation.
 * <p/>
 * <h4>Clearing the buffer indexes</h4>
 * <p/>
 * You can set both {@link #readerIndex() readerIndex} and {@link #writerIndex() writerIndex} to {@code 0} by calling
 * {@link #clear()}. It does not clear the buffer content (e.g. filling with {@code 0}) but just clears the two
 * pointers. Please also note that the semantic of this operation is different from {@link ByteBuffer#clear()}.
 * <p/>
 * 
 * <pre>
 *  BEFORE clear()
 ...
 * <p/>
 * There are two marker indexes in every buffer. One is for storing {@link #readerIndex() readerIndex} and the other is
 * for storing {@link #writerIndex() writerIndex}. You can always reposition one of the two indexes by calling a reset
 * method. It works in a similar fashion to the mark and reset methods in {@link InputStream} except that there's no
 * {@code readlimit}.
 * <p/>
 ...
 * <h4>Byte array</h4>
 * <p/>
 * If a {@link ChannelBuffer} is backed by a byte array (i.e. {@code byte[]}), you can access it directly via the
 * {@link #array()} method. To determine if a buffer is backed by a byte array, {@link #hasArray()} should be used.
 * <p/>
 * <h4>NIO Buffers</h4>
 * <p/>
 * Various {@link #toByteBuffer()} methods convert a {@link ChannelBuffer} into one or more NIO buffers. These methods
 * avoid buffer allocation and memory copy whenever possible, but there's no guarantee that memory copy will not be
 * involved.
 * <p/>
 * <h4>I/O Streams</h4>
 * <p/>
...
public interface ChannelBuffer extends Comparable<ChannelBuffer> {
     ...
     * <li>a dynamic buffer will expand its capacity so that the number of the {@link #writableBytes() writable bytes}
     * becomes equal to or greater than the specified value. The expansion involves the reallocation of the internal
     * buffer and consequently memory copy.</li>
     * </ul>
     *
     ...
     * </ul>
     * Please note that it does not compare {@link #readerIndex()} nor {@link #writerIndex()}. This method also returns
     * {@code false} for {@code null} and an object which is not an instance of {@link ChannelBuffer} type.
     */
     ...
     * {@code dstIndex} is less than {@code 0}, if {@code index + length} is greater than {@code this.capacity}, or if
     ...
     *
     * @throws IndexOutOfBoundsException if the specified {@code index} is less than {@code 0}, if the specified
     * {@code srcIndex} is less than {@code 0}, if {@code index + length} is greater than {@code this.capacity}, or if
     ...
     * {@code writerIndex} is less than the specified {@code readerIndex} or if the specified {@code writerIndex} is
     * greater than {@code this.capacity}
     */
