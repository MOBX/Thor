...
import com.mob.thor.rpc.common.bytecode.ClassGenerator;
...
import com.mob.thor.rpc.common.serialize.support.java.CompactedObjectInputStream;
import com.mob.thor.rpc.common.serialize.support.java.CompactedObjectOutputStream;
...
public abstract class Builder<T> implements GenericDataFlags {
    ...
    private static final Comparator<Field>         FC                              = new Comparator<Field>() {
                                                                                       ...
                                                                                       public int compare(Field f1,
                                                                                                          Field f2) {
                                                                                           return compareFieldName(f1.getName(),
                                                                                                                   f2.getName());
                                                                                       }
                                                                                   };
    ...
    public static ClassDescriptorMapper            DEFAULT_CLASS_DESCRIPTOR_MAPPER = new ClassDescriptorMapper() {
                                                                                       ...
                                                                                       public String getDescriptor(int index) {
                                                                                           if (index < 0
                                                                                               || index >= mDescList.size()) return null;
    ...
    public static <T> Builder<T> register(Class<T> c, boolean isAllowNonSerializable) {
        if (c == Object.class || c.isInterface()) return (Builder<T>) GenericBuilder;
        if (c == Object[].class) return (Builder<T>) GenericArrayBuilder;
        ...
        b = newBuilder(c);
        if (isSerializable) BuilderMap.put(c, b);
        else nonSerializableBuilderMap.put(c, b);
    ...
    public static <T> void register(Class<T> c, Builder<T> b) {
        if (Serializable.class.isAssignableFrom(c)) BuilderMap.put(c, b);
        else nonSerializableBuilderMap.put(c, b);
    }

    private static <T> Builder<T> newBuilder(Class<T> c) {
        if (c.isPrimitive()) throw new RuntimeException("Can not create builder for primitive type: " + c);

        if (logger.isInfoEnabled()) logger.info("create Builder for class: " + c);

        Builder<?> builder;
        if (c.isArray()) builder = newArrayBuilder(c);
        else builder = newObjectBuilder(c);
        return (Builder<T>) builder;
    }

    private static Builder<?> newArrayBuilder(Class<?> c) {
        Class<?> cc = c.getComponentType();
        if (cc.isInterface()) return GenericArrayBuilder;
        ...
        StringBuilder cwt = new StringBuilder("public void writeTo(Object obj, ").append(GenericObjectOutput.class.getName()).append(" out) throws java.io.IOException{"); // writeTo
                                                                                                                                                                           // code.
        StringBuilder cpf = new StringBuilder("public Object parseFrom(").append(GenericObjectInput.class.getName()).append(" in) throws java.io.IOException{"); // parseFrom
                                                                                                                                                                 // code.
        ...
        if (builder != null) cg.addField("public static " + BUILDER_CLASS_NAME + " builder;");
        ...
        try {
            Class<?> wc = cg.toClass();
            // set static field.
            if (builder != null) wc.getField("builder").set(null, builder);
    ...
    private static Builder<?> newObjectBuilder(final Class<?> c) {
        if (c.isEnum()) return newEnumBuilder(c);

        if (c.isAnonymousClass()) throw new RuntimeException("Can not instantiation anonymous class: " + c);

        if (c.getEnclosingClass() != null && !Modifier.isStatic(c.getModifiers())) throw new RuntimeException(
                                                                                                              "Can not instantiation inner and non-static class: "
                                                                                                                      + c);

        if (Throwable.class.isAssignableFrom(c)) return SerializableBuilder;
        ...
        if (is != null) {
            try {
                ...
                if (len > 0) {
                    if (len > MAX_FIELD_CONFIG_FILE_SIZE) throw new RuntimeException(
                                                                                     "Load ["
                                                                                             + c.getName()
        ...
        if (fns != null) {
            ...
            for (int i = 0; i < fns.length; i++) {
                ...
                try {
                    ...
                    if (Modifier.isStatic(mod) || (serializeIgnoreFinalModifier(c) && Modifier.isFinal(mod))) throw new RuntimeException(
                                                                                                                                         "Field ["
                                                                                                                                                 + c.getName()
                                                                                                                                                 + "."
                                                                                                                                                 + fn
                    ...
                    if (Modifier.isTransient(mod)) {
                        if (iss) return SerializableBuilder;
            ...
            if (fs.length > 1) Arrays.sort(fs, FC);
        }
        ...
        if (cs.length == 0) {
            ...
            do {
                t = t.getSuperclass();
                if (t == null) throw new RuntimeException("Can not found Constructor?");
                cs = c.getDeclaredConstructors();
            } while (cs.length == 0);
        }
        if (cs.length > 1) Arrays.sort(cs, CC);
        ...
        if (!dn) cni.append("}"); // close object array.
        cni.append("); }");
        ...
        for (int i = 0; i < fs.length; i++) {
            ...
            if (da) {
                ...
                pm = pms.get(fn);
                if (pm != null && (pm.type != ft || pm.setter == null || pm.getter == null)) pm = null;
            }
        ...
        if (!dn) cg.addField("public static java.lang.reflect.Constructor constructor;");
        cg.addMethod("public Class getType(){ return " + cn + ".class; }");
        ...
        try {
            ...
            if (!dn) wc.getField("constructor").set(null, con);
            return (Builder<?>) wc.newInstance();
        } catch (RuntimeException e) {
            throw e;
        } catch (Throwable e) {
    ...
    static final Builder<Object[]>     GenericArrayBuilder = new AbstractObjectBuilder<Object[]>() {
                                                               ...
                                                               @Override
                                                               protected void readObject(Object[] ret,
                                                                                         GenericObjectInput in)
                                                                                                               throws IOException {
    ...
    static final Builder<Serializable> SerializableBuilder = new Builder<Serializable>() {
                                                               ...
                                                               @Override
                                                               public void writeTo(Serializable obj,
                                                                                   GenericObjectOutput out)
                                                                                                           throws IOException {
                                                                   if (obj == null) {
                                                                       out.write0(OBJECT_NULL);
                                                                       ...
                                                                       CompactedObjectOutputStream oos = new CompactedObjectOutputStream(
                                                                                                                                         bos);
                                                                       oos.writeObject(obj);
                                                                       oos.flush();
                                                                       bos.close();
                                                               ...
                                                               public Serializable parseFrom(GenericObjectInput in)
                                                                                                                   throws IOException {
                                                                   byte b = in.read0();
                                                                   if (b == OBJECT_NULL) return null;
                                                                   if (b != OBJECT_STREAM) throw new IOException(
                                                                                                                 "Input format error, expect OBJECT_NULL|OBJECT_STREAM, get "
                                                                                                                         + b
                                                                                                                         + ".");

                                                                   UnsafeByteArrayInputStream bis = new UnsafeByteArrayInputStream(
                                                                                                                                   in.read0(in.readUInt()));
                                                                   CompactedObjectInputStream ois = new CompactedObjectInputStream(
                                                                                                                                   bis);
                                                                   try {
    ...
    static {
        ...
        register(Boolean.class, new Builder<Boolean>() {
            ...
            public void writeTo(Boolean obj, GenericObjectOutput out) throws IOException {
                if (obj == null) out.write0(VARINT_N1);
                else if (obj.booleanValue()) out.write0(VARINT_1);
                else out.write0(VARINT_0);
            }
        ...
        register(Byte.class, new Builder<Byte>() {
            ...
            public Byte parseFrom(GenericObjectInput in) throws IOException {
                byte b = in.read0();
                if (b == OBJECT_NULL) return null;
                if (b != OBJECT_VALUE) throw new IOException(
                                                             "Input format error, expect OBJECT_NULL|OBJECT_VALUE, get "
                                                                     + b + ".");
        ...
        register(ArrayList.class, new Builder<ArrayList>() {
            ...
            public ArrayList parseFrom(GenericObjectInput in) throws IOException {
                byte b = in.read0();
                if (b == OBJECT_NULL) return null;
                if (b != OBJECT_VALUES) throw new IOException(
                                                              "Input format error, expect OBJECT_NULL|OBJECT_VALUES, get "
                                                                      + b + ".");
        ...
        register(HashMap.class, new Builder<HashMap>() {
            ...
            public HashMap parseFrom(GenericObjectInput in) throws IOException {
                byte b = in.read0();
                if (b == OBJECT_NULL) return null;
                if (b != OBJECT_MAP) throw new IOException("Input format error, expect OBJECT_NULL|OBJECT_MAP, get "
                                                           + b + ".");
