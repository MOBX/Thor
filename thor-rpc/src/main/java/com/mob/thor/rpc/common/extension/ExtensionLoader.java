...
import com.mob.thor.rpc.common.extension.support.ActivateComparator;
 ...
 * @author zxc
 * @see com.mob.thor.common.extension.SPI
 * @see com.mob.thor.common.extension.Adaptive
 * @see com.mob.thor.common.extension.Activate
 */
public class ExtensionLoader<T> {
    ...
    private static final String                                      DUBBO_DIRECTORY          = "META-INF/thor/";

    private static final String                                      DUBBO_INTERNAL_DIRECTORY = DUBBO_DIRECTORY
                                                                                                + "internal/";
     ...
     * @see #getActivateExtension(com.mob.thor.common.URL, String, String)
     */
    ...
    private T createAdaptiveExtension() {
        try {
            ...
            throw new IllegalStateException(
                                            "Can not create adaptive extenstion " + type + ", cause: " + e.getMessage(),
                                            e);
        }
    }
    ...
    private Class<?> createAdaptiveExtensionClass() {
        ...
        ClassLoader classLoader = findClassLoader();
        com.mob.thor.rpc.common.compiler.Compiler compiler = ExtensionLoader.getExtensionLoader(com.mob.thor.rpc.common.compiler.Compiler.class).getAdaptiveExtension();
    ...
    private String createAdaptiveExtensionClassCode() {
        ...
        if (!hasAdaptiveAnnotation) throw new IllegalStateException("No adaptive method on extension " + type.getName()
        ...
        for (Method method : methods) {
            ...
            if (adaptiveAnnotation == null) {
                ...
                for (int i = 0; i < pts.length; ++i) {
                    if (pts[i].getName().equals("com.mob.thor.rpc.api.Invocation")) {
                        // Null Point check
                ...
                for (int i = value.length - 1; i >= 0; --i) {
                    if (i == value.length - 1) {
                        if (null != defaultExtName) {
                            if (!"protocol".equals(value[i])) if (hasInvocation) getNameCode = String.format("url.getMethodParameter(methodName, \"%s\", \"%s\")",
                                                                                                             value[i],
                                                                                                             defaultExtName);
                            else getNameCode = String.format("url.getParameter(\"%s\", \"%s\")", value[i],
                                                             defaultExtName);
                            else getNameCode = String.format("( url.getProtocol() == null ? \"%s\" : url.getProtocol() )",
                                                             defaultExtName);
                        } else {
                            if (!"protocol".equals(value[i])) if (hasInvocation) getNameCode = String.format("url.getMethodParameter(methodName, \"%s\", \"%s\")",
                                                                                                             value[i],
                                                                                                             defaultExtName);
                            else getNameCode = String.format("url.getParameter(\"%s\")", value[i]);
                            else getNameCode = "url.getProtocol()";
                        }
                    } else {
                        if (!"protocol".equals(value[i])) if (hasInvocation) getNameCode = String.format("url.getMethodParameter(methodName, \"%s\", \"%s\")",
                                                                                                         value[i],
                                                                                                         defaultExtName);
                        else getNameCode = String.format("url.getParameter(\"%s\", %s)", value[i], getNameCode);
                        else getNameCode = String.format("url.getProtocol() == null ? (%s) : url.getProtocol()",
                                                         getNameCode);
                    }
                }
