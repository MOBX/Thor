...
import com.mob.thor.rpc.remoting.api.Transporter;
import com.mob.thor.rpc.remoting.api.exchange.*;
import com.mob.thor.rpc.remoting.api.exchange.support.ExchangeHandlerAdapter;

public class ThorProtocol extends AbstractProtocol {
    ...
    private ExchangeHandler                                        requestHandler             = new ExchangeHandlerAdapter() {
                                                                                                  ...
                                                                                                  public Object reply(ExchangeChannel channel,
                                                                                                      ...
                                                                                                      if (message instanceof Invocation) {
                                                                                                          ...
                                                                                                          Invoker<?> invoker = getInvoker(channel,
                                                                                                                                          inv);
                                                                                                          // 如果是callback
                                                                                                          // 需要处理高版本调用低版本的问题
                                                                                                  ...
                                                                                                  @Override
                                                                                                  public void received(ThorChannel channel,
                                                                                                                       Object message)
                                                                                                                                      throws RemotingException {
                                                                                                      if (message instanceof Invocation) {
                                                                                                  ...
                                                                                                  public void disconnected(ThorChannel channel)
                                                                                                      ...
                                                                                                      if (logger.isInfoEnabled()) {
                                                                                                          logger.info("disconected from "
                                                                                                                      + channel.getRemoteAddress()
                                                                                                                      + ",url:"
                                                                                                                      + channel.getUrl());
                                                                                                      }
                                                                                                      invoke(channel,
                                                                                                             Constants.ON_DISCONNECT_KEY);
                                                                                                  }

                                                                                                  private void invoke(ThorChannel channel,
                                                                                                                      String methodKey) {
                                                                                                  ...
                                                                                                  private Invocation createInvocation(ThorChannel channel,
                                                                                                                                      URL url,
                                                                                                                                      String methodKey) {
                                                                                                      ...
                                                                                                      invocation.setAttachment(Constants.PATH_KEY,
                                                                                                                               url.getPath());
                                                                                                      invocation.setAttachment(Constants.GROUP_KEY,
                                                                                                                               url.getParameter(Constants.GROUP_KEY));
                                                                                                      invocation.setAttachment(Constants.INTERFACE_KEY,
                                                                                                                               url.getParameter(Constants.INTERFACE_KEY));
                                                                                                      invocation.setAttachment(Constants.VERSION_KEY,
                                                                                                                               url.getParameter(Constants.VERSION_KEY));
                                                                                                      if (url.getParameter(Constants.STUB_EVENT_KEY,
                                                                                                                           false)) {
                                                                                                          invocation.setAttachment(Constants.STUB_EVENT_KEY,
    ...
    private static ThorProtocol                                    INSTANCE;

    public ThorProtocol() {
        INSTANCE = this;
    }

    public static ThorProtocol getDubboProtocol() {
        if (INSTANCE == null) {
            ExtensionLoader.getExtensionLoader(Protocol.class).getExtension(ThorProtocol.NAME); // load
        }
        return INSTANCE;
    }
    ...
    private boolean isClientSide(ThorChannel channel) {
        InetSocketAddress address = channel.getRemoteAddress();
        URL url = channel.getUrl();
        return url.getPort() == address.getPort()
               && NetUtils.filterLocalHost(channel.getUrl().getIp()).equals(NetUtils.filterLocalHost(address.getAddress().getHostAddress()));
    }

    Invoker<?> getInvoker(ThorChannel channel, Invocation inv) throws RemotingException {
        ...
        if (exporter == null) throw new RemotingException(channel, "Not found exported service: " + serviceKey + " in "
                                                                   + exporterMap.keySet()
                                                                   + ", may be version or group mismatch "
                                                                   + ", channel: consumer: "
                                                                   + channel.getRemoteAddress() + " --> provider: "
    ...
    private ExchangeServer createServer(URL url) {
        ...
        if (str != null && str.length() > 0 && !ExtensionLoader.getExtensionLoader(Transporter.class).hasExtension(str)) throw new RpcException(
    ...
    public void destroy() {
        for (String key : new ArrayList<String>(serverMap.keySet())) {
            ...
            if (server != null) {
                try {
                    if (logger.isInfoEnabled()) {
                        logger.info("Close thor server: " + server.getLocalAddress());
                    }
        ...
        for (String key : new ArrayList<String>(referenceClientMap.keySet())) {
            ...
            if (client != null) {
                try {
                    if (logger.isInfoEnabled()) {
                        logger.info("Close thor connect: " + client.getLocalAddress() + "-->"
